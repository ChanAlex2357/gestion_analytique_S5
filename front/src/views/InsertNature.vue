<template>
    ...
    <section class="section">
      <div class="row">
        <div class="col-lg-8 offset-2 mt-5">
          <div class="card">
            <div class="card-body">
              <h5 class="card-title text-center mb-4"> Ajouter une Nature </h5>
              <div class="col-10 offset-1">
                <form @submit.prevent="submitForm">
                  <div class="row mb-3">
                    <label for="inputText" class="col-sm-2 col-form-label">Nom</label>
                    <div class="col-sm-10">
                      <input type="text" v-model="nature.name" placeholder="Insérer le nom" class="form-control" required>
                    </div>
                  </div>
  ...
  export default {
    data() {
      return {
        nature: {
          name: '' // Using 'name' to store the nature's name
        },
        modalTitle: '',
        modalMessage: ''
      };
    ...
    methods: {
      async submitForm() {
        ...
        try {
          const response = await fetch('http://localhost:8080/api/nature/insert', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
            },
            body: JSON.stringify({ name: this.nature.name }) // Use 'name' for the request
          });
          ...
          this.modalMessage = 'Nature ajoutée avec succès';
          this.showModal();
